...
- **[0.28.51] 2026-03-05**：SessionStart SystemMessage 動態注入 Plugin Context——(1) buildPluginContext() 函式從 registry.js 動態計算 agent/stage/workflow/hook 數量；(2) 組裝格式化 context 字串，包含版本號、元件概覽、核心規範、目錄結構、常用指令；(3) Main Agent systemMessage 自動注入 plugin context，失敗時靜默跳過；(4) 新增 12 個單元測試驗證動態數值與 registry 一致性；(5) 測試 +12（3366→3378）
