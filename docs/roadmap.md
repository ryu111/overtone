...
**現狀**：深度路由 D0-D4 由 Main Agent 透過 rules 判斷，metrics.js 已記錄 dispatch/error/block 數值，但缺乏任務維度的觀測和策略回饋。
**目標**：擴充 metrics.js 記錄任務維度 + 本地模型定期分析，形成「記錄（確定性）→ 分析（語意）→ 回饋」迴路。

| 任務 | 類型 | 說明 |
|------|------|------|
| metrics.js 擴充 | 修改 | 新增任務維度：depth、worker 數量、duration、成功/失敗（確定性，程式碼處理） |
| task-analyzer | 新建 | 本地模型（haiku → Qwen）定期分析 metrics 快照，產出策略建議（語意判斷） |
| 策略回饋寫入 | 新建 | 分析結果寫入 instinct 或 session context，供下次深度路由參考 |
| 觀測層測試 | 新建 | metrics 記錄正確性 + 分析觸發條件 |

> **設計決策**：
> - 深度路由判斷由 Main Agent 自行完成（LLM 語意判斷），不需要確定性腳本控制流
> - 確定性操作（記錄數值）→ 程式碼（metrics.js）
> - 語意分析（表現評估、策略建議）→ 本地模型（haiku 級，路由到 Qwen）
> - 不建立獨立 task-state.js — 擴充現有 metrics.js 避免多餘中間層
...
### R1.7 Maintainer Agent（本地模型背景 agent）

**現狀**：文件整理、Notion 同步、git commit/push 全靠人工在 session 中手動執行。
**目標**：SessionEnd 後自動背景執行，全程使用本地模型（Qwen），零 API token。

| 任務 | 類型 | 說明 |
|------|------|------|
| maintainer.js 框架 | 新建 | SessionEnd hook → `bun maintainer.js &`（背景 detached，做完自動退出） |
| 文件整理 | 新建 | roadmap 數字校驗 + 元件引用一致性 + 過時內容清理（本地模型判斷） |
| Notion 雙向同步 | 新建 | 本地進度 → Notion（SessionEnd）、Notion 任務 → 本地（SessionStart） |
| git 自動提交 | 新建 | 本地模型生成 commit message → git add + commit + push 雙 repo |
| maintainer 測試 | 新建 | 整理邏輯 + Notion API mock + git 操作驗證 |

**執行流程**：

```
SessionEnd hook → spawn maintainer.js（背景）
  1. 偵測（程式碼）— git diff + 檔案掃描
  2. 整理（本地模型）— 更新數字、修正引用、產摘要
  3. 同步（本地模型 + 程式碼）— Notion ↔ 本地
  4. 提交（本地模型 + 程式碼）— commit message → git commit + push
  → process 自然退出
```

> **設計決策**：
> - 本地模型 agent — 新型態，不經 Claude API，由 hook 觸發而非 Main Agent 委派
> - 一次性背景執行 — 非常駐服務，做完退出，不佔 server 資源
> - 語意判斷（整理/摘要/commit message）→ 本地 Qwen（`http://localhost:8000`）
> - 確定性操作（git/Notion API/檔案讀寫）→ 程式碼
> - 失敗不影響下次 session
...
| 階段 | 內容 | 可並行 |
|------|------|:------:|
| Phase 1 | R1.1 ✅ + R1.2 + R1.3 + R1.7 | R1.2 + R1.3 + R1.7 並行 |
