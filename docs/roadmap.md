...
### R1.2 Maintainer — 本地模型驅動的觀測與維護 agent（🟡 P1-P4 完成）
...
#### Phase 2：內容正確性 + 任務觀測（✅ 已完成）

| 任務 | 類型 | 狀態 | 說明 |
|------|------|:----:|------|
| Roadmap 狀態檢查 | 新建 | ✅ | 本地模型比對 R*.* 段落狀態標記是否準確 |
| metrics 分析 | 新建 | ✅ | 本地模型分析 nova-server /health metrics 快照 |
| metrics.js 擴充 | 修改 | ✅ | 新增任務維度（taskDepth、agentDelegations、sessionDuration） |

#### Phase 3：文件搬遷自動化（✅ 已完成）

| 任務 | 類型 | 狀態 | 說明 |
|------|------|:----:|------|
| backlog 完成偵測 | 新建 | ✅ | 本地模型判斷 docs/spec/ 是否已實作完成 |
| 搬遷 + 引用更新 | 新建 | ✅ | 完成 → 搬至 spec/docs/ + grep 更新引用 |

#### Phase 4：Notion 雙向同步（✅ 已完成）

| 任務 | 類型 | 狀態 | 說明 |
|------|------|:----:|------|
| Notion 推送 | 新建 | ✅ | roadmap R*.* 狀態 → Notion Status 欄位更新 |
| Notion 拉取 | 新建 | ✅ | Notion Todo 任務 → spec/change/ 本地目錄 |
| token fallback | 新建 | ✅ | env 沒有 NOTION_TOKEN 時從 ~/.zshrc 讀取 |
...
```
R1.1 守衛框架 ✅ ───────────────────────────────────┐
R1.2 Maintainer P1-P4 ✅ → P5 待 P4 穩定 ─────────┤
...
| 階段 | 內容 | 可並行 |
|------|------|:------:|
| Phase 1 | R1.1 ✅ + R1.2(P1-P4 ✅) + R1.3 ✅ | 已完成 |
