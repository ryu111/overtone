...
Layer 0: Loop（外圈）
  ...
  └─ 退出條件：checkbox 全完成 / /stop / max iterations

Layer 1: Skill + Command 引導（內圈）
  └─ Hook systemMessage（⛔ MUST）→ 觸發 /auto
  └─ /auto（workflow 選擇器 skill）→ Main Agent 判斷需求 → 選擇 workflow
...
| **UserPromptSubmit** | 注入 systemMessage 指向 /auto | ~131 |
