...
- **問題**：R4 三大子系統（工具動態組合、自驅閉環、自主適應）各有單元測試（1070 pass），但缺少端到端真實驗證。元件各自通過不代表整條鏈能跑通。
...
- 整合測試：驗證 heartbeat 模組觸發自驅閉環的完整 lifecycle
...
- [ ] `bun test` 新增整合測試全部通過
- [ ] 5 條端到端鏈路各有至少 1 個正常路徑 + 1 個錯誤路徑測試
- [ ] 不觸碰真實外部系統（Notion API、LLM、Claude CLI）
- [ ] 整合測試總執行時間 < 2 秒
- [ ] 測試檔案放在 `~/projects/overtone/tests/unit/` 下（與現有風格一致）
