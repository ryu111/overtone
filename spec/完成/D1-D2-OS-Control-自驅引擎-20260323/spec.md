...
- **問題**：現有自驅機制透過 `claude -p` spawn 獨立的非互動 session，每次冷啟動無對話上下文、只能一次性 prompt、工具權限受 `--allowedTools` 白名單限制、與手動互動 session 體驗完全不同。每個 session 消耗大量 token 重建 context（CLAUDE.md + rules + skills 重新載入），且無法多輪決策。
- **目標**：用 OS-level 自動化（iTerm2 AppleScript）在一個**活的互動 CLI session** 中自動輸入 prompt 並送出，實現等同手動操作的自驅體驗——保持對話上下文、完整 hook 體驗、多輪決策能力。
- **不做的代價**：每次自驅 session 浪費 ~5000 tokens 重建 context；無法跨任務延續上下文（分析 A 的結論無法用於 B）；工具權限受限導致部分任務無法自動完成。

## 範圍

### In-scope

- iTerm2 AppleScript 操控引擎：建立/查找自驅 session、寫入 prompt、送出
- 完成偵測機制：偵測 Claude CLI 回應完成（回到輸入等待狀態）
- 輸出讀取：透過 iTerm2 `text` 屬性讀取 session 文字內容
- 熱鍵中斷：使用者隨時關閉 OS-control 自驅的機制
...
- 非 macOS 平台支援
- 自驅 session 的視覺化 UI（Dashboard 整合留後續迭代）
- 多個同時進行的自驅 session
...
2. 身為**使用者**，我想要用熱鍵隨時中斷自驅，以便在需要時接管 session。
3. 身為**使用者**，我想要在 iTerm2 未開啟時，系統自動降級到 `claude -p` 模式，以便自驅不因環境問題停擺。
...
2. 可用 → 查找或建立專用自驅 tab/session（名為 `nova-self-drive`）
...
| 自驅 session tab 被使用者關閉 | 下次 tick 重新建立 |
| Claude CLI 在自驅 session 中 crash | 偵測到 shell prompt（非 Claude prompt），重新啟動 claude |
...
| 安全 | 自驅 session 走標準 hook 流程（guards、injector），不比現有 heartbeat 更危險 |
| 可靠性 | OS-control 失敗 100% 降級到 `claude -p`，不阻斷自驅 |
