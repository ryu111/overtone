...
- **目標**：打通 GitHub Issue → Overtone Workflow → PR 的完整閉環。使用者只需 `/issue 123` 即可啟動對應 workflow；workflow 結束後 `/pr` 自動建 PR 並連結回 Issue。
...
```
身為開發者
我想要輸入 /issue 123 就能自動讀取 Issue 內容並啟動合適的 workflow
以便不用手動複製 Issue 描述、不用思考該選哪個 workflow
```

```
身為開發者
我想要在 workflow 完成後輸入 /pr 就能自動建立 PR
...
- `/issue <number>` skill：透過 `gh issue view` 讀取 Issue → labels 映射 workflow → 建立 feature branch → 啟動 workflow
- `/pr` skill：透過 `gh pr create` 建 PR → 從 workflow 結果組裝 description → 包含 `Closes #<number>` 連結
...
### 4. `/auto` workflow 選擇器更新
   - 負責 agent：開發者
   - 相關檔案：`plugins/overtone/skills/auto/SKILL.md`
   - 說明：在工作流選擇指南表格中新增 `/issue` 和 `/pr` 的入口說明
...
### 6. 文件更新（依賴 1-4 完成）
   ...
   - 說明：更新 skill 計數、新增 `/issue` 和 `/pr` 到常用指令區塊
...
2. **feature branch 建立時機**：
   - 選項 A：`/issue` skill 執行 Step 3 時建立 → Issue skill 完整控制 git 流程
...
4. **`/pr` 是獨立工具還是 workflow 的一部分？**
   - 選項 A：獨立工具型 skill（像 `/onboard`），使用者手動觸發
   - 選項 B：整合到 DOCS stage 之後自動觸發
   - 傾向 A，因為不是所有 workflow 都需要建 PR（如 debug、diagnose）

5. **`/issue` 啟動的 workflow 是否需要注入 Issue context 到 specs？**
