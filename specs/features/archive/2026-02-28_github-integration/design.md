...
- **方案**：新增兩個工具型 Skill（`/issue`、`/pr`），純 Markdown 指引，不新增 JS 模組。依賴 `gh` CLI 做所有 GitHub API 互動，on-start.js 新增 `gh` 可用性檢查。
- **理由**：
...
| 不在 git repo 中 | `/pr` 步驟中檢查，終止並提示 |
| 已有同名 branch | `/issue` 切換到既有 branch 而非新建 |
...
interface WorkflowState {
  // ...現有欄位
  issueNumber?: number    // 關聯的 GitHub Issue number（由 /issue 設定）
}
```

此欄位由 `/issue` skill 在初始化 workflow 時，指引 Main Agent 透過 state.js 的 `updateStateAtomic` 寫入。`/pr` skill 讀取此欄位來決定是否加入 `Closes #<number>`。

## 檔案結構

```
新增的檔案：
  plugins/overtone/skills/issue/SKILL.md                      ← 新增：/issue skill 定義
  plugins/overtone/skills/issue/references/label-workflow-map.md  ← 新增：label → workflow 映射表
  plugins/overtone/skills/pr/SKILL.md                         ← 新增：/pr skill 定義
...
修改的檔案：
  ...
  plugins/overtone/skills/auto/SKILL.md                       ← 修改：工作流選擇表新增 /issue 和 /pr 入口
```
...
- **選項 A**（選擇）：`/issue` skill 的 Step 3 建立 branch
...
無 label 時：Main Agent 根據 Issue title 和 body 自行判斷（參考 `/auto` 的選擇指南）。
...
4. **`/auto` SKILL.md 更新**：在工作流選擇指南表格末尾新增兩行：
   - `/issue <number>`：從 GitHub Issue 啟動 workflow
   - `/pr`：建立 GitHub PR

5. **issueNumber 寫入方式**：Main Agent 使用 Bash 工具呼叫一段簡短的 Node.js script inline，或者直接用 `jq` 修改 workflow.json。建議用後者以避免引入新 script。實際上，更簡單的方式是讓 `/issue` skill 指引 Main Agent 在 init-workflow.js 呼叫後，用 `node -e` inline script 呼叫 `state.updateStateAtomic` 寫入 issueNumber。

6. **PR base branch 偵測**：`/pr` 不硬編碼 `main`，而是用 `gh repo view --json defaultBranchRef -q .defaultBranchRef.name` 動態取得預設分支。

7. **Specs 目錄重用**：若使用者先手動建了 specs feature（如此次的 `github-integration`），`/issue` 的 `init-workflow.js` 呼叫應傳入相同的 featureName，specs.initFeatureDir 會偵測到已存在而跳過建立。
