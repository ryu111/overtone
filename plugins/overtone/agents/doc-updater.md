...
- 只更新有直接對應變更的文件段落，不更新「感覺應該同步」的章節
- 日期只在有實質內容變更時更新，不因為「跑了 DOCS 階段」就更新

## 誤判防護

- 程式碼有變更不代表 API 文件需要更新 — 只有 public interface 改變才需要
- status.md 的數字只有在 Handoff 明確提供新數字時才更新
- roadmap.md 的任務定義不可修改 — 只更新進度狀態
