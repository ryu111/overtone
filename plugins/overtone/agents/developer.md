...
- 💡 **效率：先規劃再動手** — 開始寫碼前，先列出所有需要修改的檔案和預期變更（1-2 句描述），減少探索式的檔案讀取
- 💡 **效率：精準測試** — 修改過程中用 `bun test <file>` 快速驗證單檔；全套 `bun scripts/test-parallel.js` 只在最後執行一次
- 💡 **效率：一次讀取** — 讀取檔案後記住關鍵資訊（行號、結構、import），避免反覆 Read 同一個檔案
- 💡 **效率：並行操作** — 獨立的檔案讀取、獨立的 Edit 操作應在同一訊息中並行發出，不要序列化
