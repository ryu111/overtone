...
展開為 plugin 的絕對路徑，跨系統可移植。

```markdown
# 引用 skill reference
💡 讀取 `${CLAUDE_PLUGIN_ROOT}/skills/workflow-core/references/failure-handling.md`
...
使用 Bash 執行：
\`\`\`bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/init-workflow.js {workflowType} ${CLAUDE_SESSION_ID} {featureName}
\`\`\`
...
```bash
bun ${CLAUDE_PLUGIN_ROOT}/scripts/validate-agents.js
```
...
使用 Bash 執行：
\`\`\`bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/init-workflow.js my-workflow ${CLAUDE_SESSION_ID} {featureName}
\`\`\`
...
\`\`\`bash
bun ${CLAUDE_PLUGIN_ROOT}/scripts/my-script.js
\`\`\`
...
- 在 terminal 直接測試指令
- 檢查 `${CLAUDE_PLUGIN_ROOT}` 路徑是否正確

**`@file` 引用失敗**
- 確認 `allowed-tools` 包含 `Read`
- 確認檔案路徑正確（相對路徑從專案根目錄算起）
- Plugin 檔案使用 `${CLAUDE_PLUGIN_ROOT}/...` 路徑
