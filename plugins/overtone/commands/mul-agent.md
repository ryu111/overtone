---
name: mul-agent
description: 同類型 Agent 並行機制。將獨立子任務分批並行，縮短執行時間。適用於任何 agent 類型。
disable-model-invocation: true
---

# Mul Agent — 同類型 Agent 並行

> **何時讀取**：任何階段開始時，判斷是否有可並行的子任務。
...
2. 依 Phase 順序執行（sequential Phase 一個 agent，parallel Phase 同一訊息多個 Task）
...
3. 若有 → 在同一訊息中發多個 Task（每個子任務一個同類型 agent）
4. 若無 → 單一 agent，正常執行
...
| 標記 | 意義 |
|------|------|
| `(sequential)` | 此 Phase 的子任務依序執行（單一 agent） |
...
同類型 Agent 並行適用於任何能分解為獨立子任務的階段：

| Agent 類型 | 並行場景 | 判斷條件 |
|-----------|---------|---------|
| developer | 不同模組的功能開發 | 操作不同檔案、無邏輯依賴 |
| tester | 不同功能的測試撰寫或驗證 | 測試標的獨立、無共享 fixture |
| debugger | 多個獨立錯誤的並行診斷 | 錯誤根因在不同模組 |
| code-reviewer | 不同模組的程式碼審查 | 審查範圍無重疊 |
| security-reviewer | 不同模組的安全掃描 | 掃描標的獨立 |

💡 不適合並行的 agent：architect、planner、designer、retrospective、doc-updater — 這些角色需要全局視野，不可分割。
...
```
# TEST 階段（parallel）— 同一訊息發出兩個 Task

Task 1: 委派 tester 驗證使用者模組
  - 任務：執行 tests/user.test.ts 並驗證所有場景
  - Handoff：[developer Handoff 內容]

Task 2: 委派 tester 驗證認證模組
  - 任務：執行 tests/auth.test.ts 並驗證所有場景
  - Handoff：[developer Handoff 內容]
```
...
Task: 委派 {agent 類型} 建立基礎建設
  - 任務：依序完成 (1) 建立資料模型 (2) 設定路由骨架
  - Handoff：[前一階段 Handoff 內容]
...
Main Agent 應在相關階段的關鍵時機操作 TaskList，讓使用者即時看到執行進度。

### 時機一：階段啟動時（建立 todo items）
...
- 若分析後只有單一子任務（退化為單一 agent），仍建立一個 TaskCreate，subject：`[Phase 1] 任務描述`

### 時機二：委派 agent Task 前（標記執行中）

在同一訊息發出 Task 之前，先呼叫 TaskUpdate 將對應項目標記為執行中：

- status：`in_progress`
- `activeForm` 可選填：`執行中：子任務描述`

Parallel Phase 同時委派多個子任務時，對應的多個 TaskUpdate 也在同一訊息一起發出。

### 時機三：agent 回報完成後（標記已完成）
...
- 不需要為「整個階段」建立一個父級 TaskCreate，子任務 item 已足夠
...
4. **子任務數量為 1**：無需分拆，直接單一 agent

退化後正常執行標準的單一 agent 流程。退化時 TaskList 操作仍完整執行（TaskCreate → TaskUpdate in_progress → TaskUpdate completed），流程與一般 Phase 相同。
