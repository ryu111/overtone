'use strict';
/**
 * post-use-handler.js — PostToolUse 業務邏輯 handler
 *
 * 從 post-use.js hook 提取的純業務邏輯，供薄殼 hook 呼叫。
...
function handlePostUse(input) {
  ...
  const emitPostUseTiming = (extra = {}) =>
    hookTimer.emit(sessionId, 'post-use', 'PostToolUse', { toolName, ...extra });
  ...
  try {
    ...
    if (toolName === 'Bash') {
      ...
      if (errorGuard) {
        ...
        return { output: { result: errorGuard } };
      }
    }
  } catch {
    // 觀察失敗靜默處理，不影響工具執行
...
module.exports = { handlePostUse, observeBashError, extractCommandTag, detectWordingMismatch, WORDING_RULES };
