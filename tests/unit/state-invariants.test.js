...
describe('state 不變量守衛 — 孤兒 activeAgent 清除（規則 1）', () => {
  ...
  it('Scenario 3-1: 孤兒 activeAgent entry（stage key 不存在）被自動移除', () => {
    ...
    state.initStateCtx(new SessionContext(projectRoot, sid), 'quick', ['DEV', 'REVIEW']);

    // 注入一個 stage 為 TEST:999 的孤兒 entry（stages 中不存在）
    state.writeStateCtx(new SessionContext(projectRoot, sid), {
      ...state.readStateCtx(new SessionContext(projectRoot, sid)),
      sessionId: sid,
      activeAgents: {
        'tester:abc123': {
          agentName: 'tester',
    ...
    state.updateStateAtomicCtx(new SessionContext(projectRoot, sid), (s) => s);

    const ws = state.readStateCtx(new SessionContext(projectRoot, sid));
    // 孤兒 entry 應被移除
...
describe('state 不變量守衛 — status 逆轉阻止（規則 2）', () => {
  ...
  it('Scenario 3-3: stage status 逆轉（completedAt 存在但 status → active）被修正為 completed', () => {
    ...
    state.updateStateAtomicCtx(new SessionContext(projectRoot, sid), (s) => {
...
describe('state 不變量守衛 — 孤兒 active stage 修復（規則 4）', () => {
  ...
  it('Scenario 4-4: 多個 stage 混合，只修正孤兒 active stage', () => {
    const { projectRoot, sid } = newSession();
    state.initStateCtx(new SessionContext(projectRoot, sid), 'standard', ['DEV', 'REVIEW', 'TEST', 'RETRO']);
