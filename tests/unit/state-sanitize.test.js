...
describe('Scenario 1: session 不存在時回傳 null', () => {
  it('不存在的 sessionId 回傳 null', () => {
    ...
    const result = state.sanitizeCtx(new SessionContext(projectRoot, 'nonexistent_sanitize_session_xyz'));
...
describe('Scenario 2: 無不一致狀態時 fixed 為空陣列', () => {
  it('乾淨的 state 回傳 fixed 為空陣列', () => {
    ...
    state.initStateCtx(new SessionContext(projectRoot, id), 'quick', ['DEV', 'REVIEW']);

    const result = state.sanitizeCtx(new SessionContext(projectRoot, id));
  ...
  it('乾淨的 state 回傳 state 物件與當前 state 相同', () => {
    ...
    const before = state.readStateCtx(new SessionContext(projectRoot, id));
    const result = state.sanitizeCtx(new SessionContext(projectRoot, id));
...
describe('Scenario 3: 孤兒 activeAgent 被清除', () => {
  it('stage 不在 workflow 中的 activeAgent 被移除', () => {
    ...
    state.writeStateCtx(new SessionContext(projectRoot, id), {
      ...state.readStateCtx(new SessionContext(projectRoot, id)),
      activeAgents: {
        'tester:orphan001': {
          agentName: 'tester',
          stage: 'TEST:999',
    ...
    const ws = state.readStateCtx(new SessionContext(projectRoot, id));
  ...
  it('孤兒 activeAgent 移除後 fixed 訊息包含 instanceId', () => {
    const { projectRoot, id } = newSession();
    state.initStateCtx(new SessionContext(projectRoot, id), 'quick', ['DEV']);
...
describe('Scenario 5: completedAt 存在但 status 非 completed 時修正', () => {
  it('completedAt 存在但 status 為 active 時修正為 completed', () => {
    ...
    const s = state.readStateCtx(new SessionContext(projectRoot, id));
    s.stages['DEV'].completedAt = new Date().toISOString();
    s.stages['DEV'].status = 'active'; // 不一致：有 completedAt 卻是 active
    state.writeStateCtx(new SessionContext(projectRoot, id), s);
  ...
  it('正常的 completed stage（status 與 completedAt 一致）不被觸動', () => {
    ...
    state.updateStateAtomicCtx(new SessionContext(projectRoot, id), (s) => {
...
describe('Scenario 9: TEST:2 status=pending 但 currentStage 已推進到更後面（規則 4）', () => {
  it('驗收標準 1：TEST:2 status=active 且無 activeAgents → 修復為 pending（由規則 3 處理）', () => {
    ...
    state.initStateCtx(new SessionContext(projectRoot, id), 'standard', ['PLAN', 'ARCH', 'TEST', 'DEV', 'REVIEW', 'TEST', 'RETRO', 'DOCS']);
...
describe('Scenario 7: sanitize() 已匯出', () => {
  ...
  it('sanitize() 回傳值包含 fixed 陣列和 state 物件', () => {
    const { projectRoot, id } = newSession();
    state.initStateCtx(new SessionContext(projectRoot, id), 'single', ['DEV']);
