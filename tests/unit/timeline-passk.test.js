...
function makeSession(suffix) {
  const projectRoot = makeTmpProject(`ot-passk-${suffix}`);
...
function appendStageComplete(ctx, stage, result, ts) {
...
describe('passAtK() — Scenario D-1: 單一 stage 第一次即 pass', () => {
  ...
  beforeEach(() => {
    ...
    appendStageComplete(session.ctx, 'DEV', 'pass');
  });
  afterEach(() => {
  ...
  test('pass1 為 true', () => {
    const result = timeline.passAtKCtx(session.ctx);
...
describe('passAtK() — Scenario D-2: 前兩次 fail，第三次 pass', () => {
  ...
  beforeEach(() => {
    ...
    // 依時間順序（ts 遞增）寫入
    appendStageComplete(session.ctx, 'DEV', 'fail', '2026-01-01T00:00:00.000Z');
    appendStageComplete(session.ctx, 'DEV', 'fail', '2026-01-01T00:01:00.000Z');
    appendStageComplete(session.ctx, 'DEV', 'pass', '2026-01-01T00:02:00.000Z');
  });
  afterEach(() => {
    cleanupSession(session);
  });
...
describe('passAtK() — Scenario D-3: 多個 stage 混合結果', () => {
  ...
  beforeEach(() => {
    ...
    appendStageComplete(session.ctx, 'DEV', 'pass', '2026-01-01T00:00:00.000Z');
    // REVIEW：fail 後 pass（pass1=false, pass3=true）
    appendStageComplete(session.ctx, 'REVIEW', 'fail', '2026-01-01T00:01:00.000Z');
    appendStageComplete(session.ctx, 'REVIEW', 'pass', '2026-01-01T00:02:00.000Z');
    // TEST：3 筆 fail（pass1=false, pass3=false）
    appendStageComplete(session.ctx, 'TEST', 'fail', '2026-01-01T00:03:00.000Z');
    appendStageComplete(session.ctx, 'TEST', 'fail', '2026-01-01T00:04:00.000Z');
    appendStageComplete(session.ctx, 'TEST', 'fail', '2026-01-01T00:05:00.000Z');
  });
  afterEach(() => {
    cleanupSession(session);
  });
...
describe('passAtK() — Scenario D-5: timeline 檔案不存在', () => {
  ...
  test('不拋出例外', () => {
    expect(() => timeline.passAtKCtx(session.ctx)).not.toThrow();
  });
...
describe('passAtK() — Scenario D-8: 最後 3 次全 pass', () => {
  ...
  beforeEach(() => {
    ...
    appendStageComplete(session.ctx, 'DEV', 'fail', '2026-01-01T00:00:00.000Z');
    appendStageComplete(session.ctx, 'DEV', 'pass', '2026-01-01T00:01:00.000Z');
    appendStageComplete(session.ctx, 'DEV', 'pass', '2026-01-01T00:02:00.000Z');
    appendStageComplete(session.ctx, 'DEV', 'pass', '2026-01-01T00:03:00.000Z');
  });
  afterEach(() => {
    cleanupSession(session);
  });
