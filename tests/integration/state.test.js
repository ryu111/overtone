...
beforeEach(() => {
  PROJECT_ROOT = mkdtempSync(join(tmpdir(), 'state-integ-'));
  SESSION_DIR = paths.sessionDir(PROJECT_ROOT, TEST_SESSION);
...
describe('setFeatureName', () => {
  ...
  test('不存在的 session 靜默忽略（不拋出）', () => {
    expect(() => state.setFeatureName(PROJECT_ROOT, 'nonexistent_session', null, 'my-feature')).not.toThrow();
  });
