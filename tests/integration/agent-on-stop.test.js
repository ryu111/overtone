...
describe('場景 8：並行收斂偵測', () => {
  test('REVIEW 和 TEST 都 active，REVIEW 先完成 → TEST 仍為 active（未全部完成）', async () => {
    ...
    state.updateStateAtomic(sessionId, (s) => {
      ...
      s.activeAgents['reviewer:1'] = { agentName: 'ot:code-reviewer', stage: 'REVIEW', startedAt: new Date().toISOString() };
      s.activeAgents['tester:1'] = { agentName: 'ot:tester', stage: 'TEST', startedAt: new Date().toISOString() };
      return s;
    });
