...
// per-project 測試根目錄（session 目錄 + specs 目錄隔離）
const TEST_PRJ = join(homedir(), '.nova', 'test-tmp', `pre-compact-prj-${TS}`);
const TMP_ROOT = TEST_PRJ; // specs 目錄也放在此
 ...
 */
function readTimeline(projectRoot, sessionId) {
  const timelinePath = paths.session.timeline(projectRoot, sessionId);
 ...
 */
function buildCtxFromSession(projectRoot, sessionId) {
  const currentState = stateLib.readState(projectRoot, sessionId);
...
afterAll(() => {
  // per-project 架構下，session 資料在各自的 projectRoot 下，清理 projectRoot 即可
  rmSync(TEST_PRJ, { recursive: true, force: true });
...
describe('Feature 1：無 sessionId 時靜默退出', () => {
  ...
  test('session_id 存在但 workflow.json 不存在時輸出 {"result":""}', () => {
    ...
    const result = runHook({ session_id: SESSION_NO_WF, cwd: TEST_PRJ });
    expect(result.exitCode).toBe(0);
    expect(JSON.parse(result.stdout)).toEqual({});
  });
});
...
describe('Feature 2：有 workflow state 時組裝狀態摘要', () => {
  beforeAll(() => {
    ...
    stateLib.initState(TEST_PRJ, SESSION_MAIN, 'standard', STANDARD_STAGES);
    // 設定 PLAN 和 ARCH 為 completed
    stateLib.updateStage(TEST_PRJ, SESSION_MAIN, null, 'PLAN', { status: 'completed', result: 'pass' });
    stateLib.updateStage(TEST_PRJ, SESSION_MAIN, null, 'ARCH', { status: 'completed', result: 'pass' });
  });

  // Scenario 2.1 — buildCompactMessage 直接驗證 systemMessage 內容
  test('systemMessage 首行為 [Overtone 狀態恢復（compact 後）]', () => {
    const ctx = buildCtxFromSession(TEST_PRJ, SESSION_MAIN);
  ...
  test('failCount > 0 時 systemMessage 包含失敗次數', () => {
    const currentState = stateLib.readState(TEST_PRJ, SESSION_MAIN);
  ...
  test('pre-compact 執行後 activeAgents 被清空（壓縮前殘留的 entry 應清除）', () => {
    const ws = stateLib.readState(TEST_PRJ, SESSION_MAIN);
    ws.activeAgents = { developer: { agentName: 'developer', stage: 'DEV', startedAt: new Date().toISOString() } };
    stateLib.writeState(TEST_PRJ, SESSION_MAIN, ws);
    const result = runHook({ session_id: SESSION_MAIN, cwd: TEST_PRJ });
    JSON.parse(result.stdout); // 確保 JSON 解析成功
    // pre-compact 應清空 activeAgents
    const wsAfter = stateLib.readState(TEST_PRJ, SESSION_MAIN);
...
describe('Feature 3：有活躍 feature 時注入未完成任務清單', () => {
  ...
  beforeAll(() => {
    stateLib.initState(TEST_PRJ, SESSION_FEAT, 'quick', ['DEV', 'REVIEW', 'TEST', 'RETRO']);
  ...
  test('有未完成任務時 systemMessage 包含 feature 名稱統計行', () => {
    const ctx = buildCtxFromSession(TMP_ROOT, SESSION_FEAT);
    const systemMessage = buildCompactMessage(ctx);
  ...
  test('超過 5 個未完成任務時只顯示前 5 個 + 剩餘計數', () => {
    ...
    stateLib.initState(ROOT_8, SESSION_8, 'quick', ['DEV', 'REVIEW', 'TEST', 'RETRO']);
    ...
    const ctx = buildCtxFromSession(ROOT_8, SESSION_8);
    const systemMessage = buildCompactMessage(ctx);
    // 前 5 個出現，第 6 個不出現
  ...
  test('所有任務完成時 systemMessage 不包含未完成任務段落', () => {
    stateLib.initState(TMP_ROOT_EMPTY, SESSION_EMPTY_TASKS, 'quick', ['DEV', 'REVIEW', 'TEST', 'RETRO']);
    createFeature(TMP_ROOT_EMPTY, 'all-done', ['- [x] 任務 A', '- [x] 任務 B']);
    const ctx = buildCtxFromSession(TMP_ROOT_EMPTY, SESSION_EMPTY_TASKS);
    const systemMessage = buildCompactMessage(ctx);
  ...
  test('無活躍 feature 時 systemMessage 不包含未完成任務段落', () => {
    ...
    stateLib.initState(ROOT_EMPTY_DIR, SESSION_NO_FEAT, 'quick', ['DEV', 'REVIEW', 'TEST', 'RETRO']);
    mkdirSync(join(ROOT_EMPTY_DIR, 'specs', 'features', 'in-progress'), { recursive: true });
    const ctx = buildCtxFromSession(ROOT_EMPTY_DIR, SESSION_NO_FEAT);
    const systemMessage = buildCompactMessage(ctx);
    expect(systemMessage).toBeDefined();
    expect(systemMessage).not.toContain('📋 **未完成任務**');
    // 清理
  ...
  test('specs 讀取失敗時仍輸出 workflow 摘要（不拋錯）', () => {
    ...
    const SPEC_ERR_ROOT = join(homedir(), '.nova', 'test-tmp', `pre-compact-spec-err-${TS}`);
    stateLib.initState(SPEC_ERR_ROOT, SESSION_SPEC_ERR, 'quick', ['DEV', 'REVIEW', 'TEST', 'RETRO']);
    // 傳入不存在的 projectRoot，buildPendingTasksMessage 內部有 try/catch 會回傳 null
    const ctx = buildCtxFromSession(SPEC_ERR_ROOT, SESSION_SPEC_ERR);
    const systemMessage = buildCompactMessage(ctx);
    expect(systemMessage).toBeDefined();
    expect(systemMessage).toContain('[Overtone 狀態恢復（compact 後）]');
    expect(systemMessage).not.toContain('📋 **未完成任務**');
    // 清理
    rmSync(SPEC_ERR_ROOT, { recursive: true, force: true });
  });
});
...
describe('Feature 4：systemMessage 長度截斷保護', () => {
  ...
  test('正常 systemMessage 不含截斷提示', () => {
    ...
    const ROOT_SHORT = join(homedir(), '.nova', 'test-tmp', `pre-compact-short-${TS}`);
    stateLib.initState(ROOT_SHORT, SESSION_SHORT, 'single', ['DEV']);
    const ctx = buildCtxFromSession(ROOT_SHORT, SESSION_SHORT);
    const systemMessage = buildCompactMessage(ctx);
    expect(systemMessage).not.toContain('已截斷，完整狀態請查看');
    rmSync(ROOT_SHORT, { recursive: true, force: true });
  });
  ...
  test('systemMessage 超過 2000 字元時截斷並附提示', () => {
    ...
    stateLib.initState(ROOT_LONG, SESSION_LONG, 'full', ['PLAN', 'ARCH', 'DESIGN', 'TEST', 'DEV', 'REVIEW', 'TEST', 'QA', 'E2E', 'RETRO', 'DOCS']);
    ...
    const ctx = buildCtxFromSession(ROOT_LONG, SESSION_LONG);
    const systemMessage = buildCompactMessage(ctx);
...
describe('Feature 5：timeline 事件 session:compact', () => {
  ...
  test('有 workflow 時 timeline.jsonl 新增 session:compact 事件（含 trigger 診斷欄位）', () => {
    ...
    const ROOT_TL = join(homedir(), '.nova', 'test-tmp', `pre-compact-tl-${TS}`);
    stateLib.initState(ROOT_TL, SESSION_TL, 'quick', ['DEV', 'REVIEW', 'TEST', 'RETRO']);
    const result = runHook({ session_id: SESSION_TL, cwd: ROOT_TL });
    expect(result.exitCode).toBe(0);

    const events = readTimeline(ROOT_TL, SESSION_TL);
    ...
    rmSync(ROOT_TL, { recursive: true, force: true });
  });
  ...
  test('trigger=auto 時 timeline 事件記錄 trigger=auto, counted=auto', () => {
    ...
    const ROOT_TL_AUTO = join(homedir(), '.nova', 'test-tmp', `pre-compact-tl-auto-${TS}`);
    stateLib.initState(ROOT_TL_AUTO, SESSION_TL_AUTO, 'quick', ['DEV', 'REVIEW', 'TEST', 'RETRO']);
    runHook({ session_id: SESSION_TL_AUTO, cwd: ROOT_TL_AUTO, trigger: 'auto' });
    const events = readTimeline(ROOT_TL_AUTO, SESSION_TL_AUTO);
    const compactEvent = events.find(e => e.type === 'session:compact');
    expect(compactEvent.trigger).toBe('auto');
    expect(compactEvent.counted).toBe('auto');
    rmSync(ROOT_TL_AUTO, { recursive: true, force: true });
  });

  test('trigger=manual 時 timeline 事件記錄 trigger=manual, counted=manual', () => {
    const SESSION_TL_MAN = `pre-compact-tl-man-${TS}`;
    const ROOT_TL_MAN = join(homedir(), '.nova', 'test-tmp', `pre-compact-tl-man-${TS}`);
    stateLib.initState(ROOT_TL_MAN, SESSION_TL_MAN, 'quick', ['DEV', 'REVIEW', 'TEST', 'RETRO']);
    runHook({ session_id: SESSION_TL_MAN, cwd: ROOT_TL_MAN, trigger: 'manual' });
    const events = readTimeline(ROOT_TL_MAN, SESSION_TL_MAN);
    const compactEvent = events.find(e => e.type === 'session:compact');
    expect(compactEvent.trigger).toBe('manual');
    expect(compactEvent.counted).toBe('manual');
    rmSync(ROOT_TL_MAN, { recursive: true, force: true });
  });

  test('無 trigger 欄位時 timeline 記錄 trigger=(empty), counted=auto', () => {
    const SESSION_TL_NONE = `pre-compact-tl-none-${TS}`;
    const ROOT_TL_NONE = join(homedir(), '.nova', 'test-tmp', `pre-compact-tl-none-${TS}`);
    stateLib.initState(ROOT_TL_NONE, SESSION_TL_NONE, 'quick', ['DEV', 'REVIEW', 'TEST', 'RETRO']);
    runHook({ session_id: SESSION_TL_NONE, cwd: ROOT_TL_NONE });
    const events = readTimeline(ROOT_TL_NONE, SESSION_TL_NONE);
    const compactEvent = events.find(e => e.type === 'session:compact');
    expect(compactEvent.trigger).toBe('(empty)');
    expect(compactEvent.counted).toBe('auto');
    rmSync(ROOT_TL_NONE, { recursive: true, force: true });
  });
  ...
  test('無 workflow 時不 emit session:compact 事件', () => {
    // SESSION_NO_WF2 從未初始化 workflow.json
    const SESSION_NO_WF2 = `pre-compact-no-wf2-${TS}`;
    const ROOT_NO_WF2 = join(homedir(), '.nova', 'test-tmp', `pre-compact-no-wf2-${TS}`);
    runHook({ session_id: SESSION_NO_WF2, cwd: ROOT_NO_WF2 });
    const events = readTimeline(ROOT_NO_WF2, SESSION_NO_WF2);
    const compactEvent = events.find(e => e.type === 'session:compact');
    expect(compactEvent).toBeUndefined();
    rmSync(ROOT_NO_WF2, { recursive: true, force: true });
  });
});
...
describe('Feature 6：任何失敗 fallback 到 { result: \'\' }', () => {
  ...
  test('workflow.json 損壞時 fallback 到 {"result":""}', () => {
    ...
    const ROOT_CORRUPT = join(homedir(), '.nova', 'test-tmp', `pre-compact-corrupt-${TS}`);
    // 手動建立損壞的 workflow.json
    const sessionDir = paths.sessionDir(ROOT_CORRUPT, SESSION_CORRUPT);
    mkdirSync(sessionDir, { recursive: true });
    writeFileSync(join(sessionDir, 'workflow.json'), '{broken json');

    const result = runHook({ session_id: SESSION_CORRUPT, cwd: ROOT_CORRUPT });
    expect(result.exitCode).toBe(0);
    expect(JSON.parse(result.stdout)).toEqual({});

    rmSync(ROOT_CORRUPT, { recursive: true, force: true });
  });
