...
function writeEvent(sessionId, stage, result, ts) {
  const filePath = paths.session.timeline(homedir(), sessionId);
...
describe('emit()', () => {
  ...
  test('正常寫入後 query 能讀回同一筆事件', () => {
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'workflow:start', { workflowType: 'single' });
    const events = timeline.queryCtx(new SessionContext(homedir(), sessionId));
  ...
  test('寫入的事件包含 ts、type、category 欄位', () => {
    const returned = timeline.emitCtx(new SessionContext(homedir(), sessionId), 'stage:start', { stage: 'DEV' });
    // 回傳值驗證
  ...
  test('未知的 eventType 應拋出 Error', () => {
    expect(() => {
      timeline.emitCtx(new SessionContext(homedir(), sessionId), 'totally:unknown:event');
    }).toThrow('未知的 timeline 事件類型：totally:unknown:event');
  });

  // Scenario: 多次 emit — 事件依序追加，query 回傳正確數量
  test('多次 emit 後 query 回傳正確數量', () => {
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'workflow:start');
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'stage:start',    { stage: 'DEV' });
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'agent:delegate', { agent: 'developer' });
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'stage:complete', { stage: 'DEV', result: 'pass' });
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'workflow:complete');
...
describe('query()', () => {
  ...
  beforeEach(() => {
    ...
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'stage:complete', { stage: 'DEV', result: 'pass' });
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'agent:delegate', { agent: 'developer' });
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'agent:complete', { agent: 'developer' });
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'workflow:complete');
  });
  ...
  test('type filter 只回傳指定類型事件', () => {
    const agentEvents = timeline.queryCtx(new SessionContext(homedir(), sessionId), { type: 'agent:delegate' });
    expect(agentEvents.length).toBe(1);
    expect(agentEvents[0].type).toBe('agent:delegate');
    expect(agentEvents[0].agent).toBe('developer');

    const stageEvents = timeline.queryCtx(new SessionContext(homedir(), sessionId), { type: 'stage:complete' });
  ...
  test('category filter 只回傳指定分類事件', () => {
    const workflowEvents = timeline.queryCtx(new SessionContext(homedir(), sessionId), { category: 'workflow' });
    expect(workflowEvents.length).toBe(2);
    workflowEvents.forEach((e) => expect(e.category).toBe('workflow'));

    const stageEvents = timeline.queryCtx(new SessionContext(homedir(), sessionId), { category: 'stage' });
    expect(stageEvents.length).toBe(2);
    stageEvents.forEach((e) => expect(e.category).toBe('stage'));

    const agentEvents = timeline.queryCtx(new SessionContext(homedir(), sessionId), { category: 'agent' });
    expect(agentEvents.length).toBe(2);
  });

  // Scenario: limit — 回傳最多 N 筆（從最新開始）
  test('limit 回傳最後 N 筆事件', () => {
    const last2 = timeline.queryCtx(new SessionContext(homedir(), sessionId), { limit: 2 });
  ...
  test('不存在的 session 回傳空陣列', () => {
    const events = timeline.queryCtx(new SessionContext(homedir(), 'nonexistent_session_xyz_999'));
...
describe('latest()', () => {
  ...
  test('有匹配事件時回傳最新一筆', () => {
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'stage:complete', { stage: 'DEV', result: 'fail' });
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'stage:complete', { stage: 'DEV', result: 'pass' });

    const result = timeline.latestCtx(new SessionContext(homedir(), sessionId), 'stage:complete');
  ...
  test('無匹配事件時回傳 null', () => {
    timeline.emitCtx(new SessionContext(homedir(), sessionId), 'workflow:start');
    const result = timeline.latestCtx(new SessionContext(homedir(), sessionId), 'workflow:complete');
...
describe('passAtK()', () => {
  ...
  test('空 timeline — stages 為 {} 且 overall rates 為 null', () => {
    const result = timeline.passAtKCtx(new SessionContext(homedir(), TEST_SESSION));
