...
├── scripts/lib/    # 共用庫（51 個模組：registry, state, timeline, specs, config-api, hook-timing, feature-sync, specs-archive-scanner, statusline-state, 3x knowledge 模組、9x handler 等）
└── web/            # Dashboard 前端
...
# 系統健康檢查（17 項偵測）— （已包含所有項目）
