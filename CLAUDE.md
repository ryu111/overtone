...
├── skills/         # 26 個 Skill（WHAT — 知識域 + thinking + orchestrator + utilities-with-refs + instinct + skill-judge）
...
# 驗證所有元件設定（18 agents + 11 hooks + 26 skills + 29 commands）
...
# Skill 品質評分（程式化評分，輸出 JSON）
bun ~/.claude/scripts/skill-score.js <skill-name>
bun ~/.claude/scripts/skill-score.js --all
```
